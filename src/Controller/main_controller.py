...
class MainController(MainView):
    ...
    def undo_move(self):
        movement = self.movements.undo()
        if movement is not None:
            piece = self.game.pieces[movement[2][0]][movement[2][1]]
            target = self.game.pieces[movement[1][0]][movement[1][1]]
            if movement[3]:
                captured = None
    ...
    def redo_move(self):
        movement = self.movements.redo()
        if movement is not None:
            piece = self.game.pieces[movement[1][0]][movement[1][1]]
            target = self.game.pieces[movement[2][0]][movement[2][1]]
            self.game.move_piece(target, piece, True)
